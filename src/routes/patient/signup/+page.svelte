<script lang="ts">
	...
	// Function to update dob whenever dobString changes
	$: {
		if (dobString) {
			...
			if (!isNaN(dateObj.getTime())) {
				...
				dob = null; // Handle invalid date gracefully
			}
		} else {
			dob = null;
		}
	...
	async function signUp() {
		try {
			const responce = await fetch('/patient/signup', {
				method: 'POST',
				body: JSON.stringify({ name, telephone, password, dob, state, gender }),
				headers: {
	...
	async function updateProfilePic() {
		try {
			...
			const responce = await fetch('/patient/signup/avatar/', {
				method: 'PUT',
				body: formData
			});
